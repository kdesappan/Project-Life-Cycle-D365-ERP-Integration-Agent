...
if "_extraction_error" in ifrs15_data:
    ...
    if lines:
        ...
        print(f"OK BillingMethod: {line.get('BillingMethod', 'MISSING')} (should be 'FixedPriceMilestone')")
        print(f"OK RevenueRecognitionMethod: {line.get('RevenueRecognitionMethod', 'MISSING')}")
        print(f"OK ProgressMeasurement: {line.get('ProgressMeasurement', 'MISSING')}")
        print(f"OK ContractLineNumber: {line.get('ContractLineNumber', 'MISSING')}")
        print(f"OK LineAmountExcludingTax: {line.get('LineAmountExcludingTax', 'MISSING')}")
        print(f"OK DataAreaId: {line.get('dataAreaId', 'MISSING')}")
...
if "_extraction_error" in milestone_data:
    ...
    if schedules:
        sched = schedules[0]
        print(f"OK Contract ID: {sched.get('ProjectContractId', 'MISSING')}")
        print(f"OK BillingScheduleId: {sched.get('BillingScheduleId', 'MISSING')}")
        print(f"OK DataAreaId: {sched.get('dataAreaId', 'MISSING')}")
        print(f"OK Total Amount: {sched.get('TotalAmountExcludingTax', 'MISSING')}")
    
    if lines:
        line = lines[0]
        print(f"\nFirst milestone:")
        print(f"OK MilestoneId: {line.get('MilestoneId', 'MISSING')}")
        print(f"OK BillingScheduleId: {line.get('BillingScheduleId', 'MISSING')}")
        print(f"OK Amount: {line.get('AmountExcludingTax', 'MISSING')}")
        print(f"OK DataAreaId: {line.get('dataAreaId', 'MISSING')}")
        print(f"OK Total milestones: {len(lines)}")
    else:
        print("ERROR: No ProjectBillingScheduleLines found")
