...
wait_step(session_id, "project_contract", "awaiting_approval")
...
step2 = approve(session_id, "project_contract", "create project contract header/funding")
print_calls(step2, "project_contract")

if (step2.get("status") or "").lower() == "failed":
    raise SystemExit(f"project_contract failed: {step2.get('error_message')}")

print("\nproject_contract succeeded")
